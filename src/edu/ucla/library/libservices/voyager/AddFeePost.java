...
public class AddFeePost
{
    ...
    throws IOException, ParseException
  {
      ...
      )
    {
      ...
      for (Iterator it = lines.iterator (); it.hasNext (); ) 
      {
        ...
        if (tokens.length >= 5)
        {
          ...
          String postAmount = tokens[1];
          String fineFeeTransactionType = tokens[2];
          String fineFeeTransactionMethod = tokens[3];
          String fineFeeId = tokens[4];
					String note = "";
					if (tokens.length >= 6) 
					{
					  note = tokens[5];
					}
          int returnCode = cc.addFeePost(patronId, postAmount, fineFeeTransactionType, fineFeeTransactionMethod, fineFeeId, note);
          if (returnCode != 0)
          {
    ...
    else
    {
      ...
      formatter.printHelp( "AddFeePost", options);
    }
  }
}
